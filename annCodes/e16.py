...
Lx = [0, h, 2*h, 3*h, 4*h] # coordenadas dos pontos em relacão a x
Ly = [f(0), f(h), f(2*h), f(3*h), f(4*h)] # coordenadas dos pontos em relacão a y
...
def lagrangePoli(x, h): # crio uma função a partir desse polinômio, copiei e colei o que me foi retornado da função Lagrange
    return -x*(-4*h + x)*(-3*h + x)*(-2*h + x)*exp(h)/(6*h**3) + x*(-4*h + x)*(-3*h + x)*(-h + x)*exp(2*h)/(2*h**3) - x*(-4*h + x)*(-2*h + x)*(-h + x)*exp(3*h)/(2*h**3) + x*(-3*h + x)*(-2*h + x)*(-h + x)*exp(4*h)/(6*h**3)
