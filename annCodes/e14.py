...
def formulaCincoPontos(p, h):
    return ((-1*f(p-2*h) + 16*f(p-h) -30*f(p) +16*f(p+h)-f(p+2*h))/(12*h*h)) # retorna o valor obtido na formula de 3 pontos

def erro(p, h):
    return abs(fderivada(p)-formulaCincoPontos(p, h)) # função para cálculo do erro
